...
@implementation NEXJSONExporter
                    ...
                    if([property isKindOfClass:[NSAttributeDescription class]]) {
                        ...
                        // value type not directly representable in JSON
                            ...
                            if(attrType == NSDateAttributeType) {
                                ...
                                [attrs setValue:@{kValueKey:[formatter stringFromDate:dateVal],
                                ...
                                [attrs setValue:@{kValueKey:[dat base64EncodedDataWithOptions:0],
                                                  kClassKey:klassName}
                                         forKey:[property name]];
                            }
                            else if(attrType == NSTransformableAttributeType) {
#warning Making that assumption that all transformables are UIImage
                                
                                NSValueTransformer *transformer = [TNImageToDataTransformer new];
                                NSData *transformed = [[transformer transformedValue:val] base64EncodedDataWithOptions:0];
                                NSString *base64String = [[NSString alloc] initWithData:transformed encoding:NSUTF8StringEncoding];
                                
                                [attrs setValue:@{kValueKey:base64String} // a dictionary as value without a value for kClassKey implies a transformed val
                                         forKey:[property name]];
                            }
                            else {
                    ...
                    for(NSString *attrName in attrs) {
                        ...
                        if(attr == [NSNull null]) {
                            ...
                            if([(NSDictionary *)attr objectForKey:kClassKey] == nil) {
                                NSData *dat = [[NSData alloc] initWithBase64EncodedString:val options:0];
                                ...
                                TNImageToDataTransformer *transformer = [TNImageToDataTransformer new];
